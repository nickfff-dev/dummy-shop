  ...
  return (
    <div className="bg-white -mt-6">
      <div className="mx-auto max-w-2xl py-8 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className="flex justify-between items-center"><h2 className="text-[28px] font-bold tracking-tight  text-gray-900">FSA & HSA Eligible</h2>
          <div className="flex ">
        ...
        <div className="mt-12 grid grid-cols-1 gap-y-8 gap-x-6 sm:grid-cols-2 lg:grid-cols-6 xl:gap-x-3 ">
          {products.map((product:any, index:number) => (
            <Product key={index} product={product} addTocart={addTocart} removeFromCart={removeFromCart} products={ products} />
          ))}
        </div>
      </div>
    </div>
